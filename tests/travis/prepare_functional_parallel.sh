...
test_set_list=$(grep "php72" -Rl src/Test/Functional/Acceptance | sort)
test_set_count=$(printf "$test_set_list" | wc -l)
let "test_set_count++"

test_set_size[1]=$(printf "%.0f" $(echo "$test_set_count*0.33" | bc))
test_set_size[2]=$(printf "%.0f" $(echo "$test_set_count*0.33" | bc))
